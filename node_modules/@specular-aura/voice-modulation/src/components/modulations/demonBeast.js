...
async function demonBeastTransform(audioBuffer, distortionAmount = 100) {
  ...
  convolver.buffer = await ctx.decodeAudioData(
    await (
      await fetch("/audio/impulse-responses/voxengo/Large Wide Echo Hall.wav")
    ).arrayBuffer()
  );
  ...
  fire.buffer = await ctx.decodeAudioData(
    await (
      await fetch("/audio/backgrounds/brush_fire-Stephan_Schutze-55390065.mp3")
    ).arrayBuffer()
  );
  fire.loop = true;
...
export default demonBeastTransform;
